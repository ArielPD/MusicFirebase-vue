      ...
      <div class="mb-4" v-for="upload in uploads" :key="upload.name">
        ...
        <div class="flex h-4 overflow-hidden bg-gray-200 rounded">
          ...
          <div
            ...
            :style="{ width: upload.current_progess + '%' }"
          ></div>
        </div>
      </div>
      <!-- <div class="mb-4">
  ...
  methods: {
    upload($event) {
      ...
      files.forEach((file) => {
        ...
        const storageRefSongs = ref(storage, `songs/${file.name}`);
        /*uploadBytes(storageRefSongs, file).then(() => {
          console.log("Upload a blob or file!");
        });*/

        const uploadTask = uploadBytesResumable(storageRefSongs, file);
